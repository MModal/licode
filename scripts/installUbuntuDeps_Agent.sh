...
install_apt_deps(){
  ...
  nvm use
  npm install
  npm install -g node-gyp gulp-cli webpack gulp gulp-eslint@3 run-sequence webpack-stream google-closure-compiler-js del gulp-sourcemaps
  npm install script-loader expose-loader
  sudo apt-get install -qq python-software-properties -y
  sudo apt-get install -qq software-properties-common -y
  sudo add-apt-repository ppa:ubuntu-toolchain-r/test -y
  sudo apt-get update -y
  sudo apt-get install -qq git make gcc-5 g++-5 libssl-dev cmake libglib2.0-dev pkg-config libboost-regex-dev libboost-thread-dev libboost-system-dev liblog4cxx10-dev libboost-test-dev -y
...
install_mediadeps(){
  ...
  if [ -d $LIB_DIR ]; then
    cd $LIB_DIR
    if [ ! -f ./v11.10.tar.gz ]; then
      curl -O -L https://github.com/libav/libav/archive/v11.10.tar.gz
      tar -zxvf v11.10.tar.gz
      cd libav-11.10
